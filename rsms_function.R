func <- function(parameters) {
  ...
  makeVar2<-function() {
    d<-lapply(1:nSpecies,function(x) {
      matrix(0,nrow=info[x,'la'],ncol=nYears.model,dimnames=list(a=1:info[x,'la'],y=years.model))
    })
    names(d)<-spNames
    d
  }
  makeVar2sp<-function() {
    ...
    names(d)<- paste(rep(1:nYears.model,each=nSeasons),rep(1:nSeasons,times=nYears.model),sep='_')
    d
  }
  ...
  makeVar3<-function() {
    d<-lapply(1:nSpecies,function(x) {
      array(0,dim=c(nYears.model,nSeasons,info[x,'la']),dimnames=list(y=years.model,q=1:nSeasons,a=1:info[x,'la']))
    })
    names(d)<-spNames
    d
  }
  ...
  makeVar3allLogical<-function() {
    d<-lapply(1:(nSpecies+nOthSpecies),function(x) {
      array(FALSE,dim=c(nYears.model,nSeasons,info[x,'la']),dimnames=list(y=years.model,q=1:nSeasons,a=1:info[x,'la']))
    })
    if (nOthSpecies>0) names(d)<-c(spNames,othspNames) else names(d)<-spNames
    d
  }
  ...
  logF<-list()
  for (s in seq_len(nSpecies)) if (nlogFfromTo[s,1]>0) logF<-c(logF, list(Uf[(nlogFfromTo[s,1]:nlogFfromTo[s,2]),,drop=FALSE])) else logF<-c(logF,NA)
  
  #timeSteps <- nYears.model
  stateDimF <- nlogF
  stateDimN <- nlogN
  ...
  for (s in seq_along(MM)) MM[[s]]<-natMor[[s]]
  
  noCatch<-0
  Chat<-makeVar3()
  for (s in seq_along(Chat)) Chat[[s]][,,]<-noCatch
  
  predN<-makeVar2()

  ssb<-    matrix(0,nrow=nSpecies,ncol=nYears.model,dimnames=list(species=spNames,year=years.model))
  Fbar<-  matrix(0,nrow=nSpecies,ncol=nYears.model,dimnames=list(species=spNames,year=years.model))
  FbarAnn<-matrix(0,nrow=nSpecies,ncol=nYears.model,dimnames=list(species=spNames,year=years.model))
  recruit<-matrix(0,nrow=nSpecies,ncol=nYears.model,dimnames=list(species=spNames,year=years.model))
  ...
  SSB_R<-function(s,y,a=1) {
    choice<-as.character(stockRecruitmentModelCode[s])
    if (length(y)==1) if (!recruitYears[s,y]) choice<-"0"
    switch( choice,
...
#   
  ...
  for (s in seq_len(nSpecies)) {
    ...
    for (y in seq_len(nYears.model)) {
      for (a in (info[s,'faf']:info[s,'la'])) {
        for (q in fqa[a]:lq)  {
          if (seasFprop[[s]][y,q,a]>0) {
            if (useFrandom) {
              cat(y,q,a,length(logF[[s]][keyLogFsta[s,a],y])==length( FisQ[[s]][y,q,a]),'\n')
              FisQ[[s]][y,q,a]<- logF[[s]][keyLogFsta[s,a],y] 
            } else FisQ[[s]][y,q,a]<-logYearEffectF[idxFY,y] 
    ...
    } else { #no process errors on N, recruits in all years, and oldest ages in first year
  ...
  for (y in seq_len(nYears.model)) {
    for (q in (1:nSeasons)) {
      ...
      if (sms.mode>0) {
        if (useNbar==1)  {
          #calc_Z_Nbar(y,q,yq,iter=1)
           for (s in seq_len(nSpecies))  {
             if (isPrey[s]) MM[[s]][y,q,]<-natMor1[[s]][y,q,]+M2[[yq]][s,1:la[s]]
             Zq[[s]][y,q,]<-MM[[s]][y,q,]+FisQ[[s]][y,q,]
             logNbarq[[s]][y,q,]<-logNq[[s]][y,q,]-log(Zq[[s]][y,q,]) + log(1.0 -exp(-Zq[[s]][y,q,]))
           }
           M2[[yq]][,]<-calc_M2(yq,N=logNbarq)
       } else  M2[[yq]][,]<-calc_M2(yq,N=logNq)
      } 
      ...
      for (s in seq_len(nSpecies)) {
        ...
        for (a in (faq[q]:la[s])) {
          if (keyLogFsta[s,a]>0) Chat[[s]][y,q,a] <- exp(logNbarq[[s]][y,q,a])*FisQ[[s]][y,q,a] 

          # N next season or year'
          if (q==lq) { # birthday for non state N species
            if (!doProcessN_old[s] &  y <nYears.model) {
              if (a <la[s]) {
 ...
 for (s in seq_len(nSpecies)) {  
  if (doProcessN_any[s]) {
   ...
   if (stockRecruitmentModelCode[s] >=1 & recAge==0 & recruitYears[s,1]){
    ...
    for (y in seq.int(2L,nYears.model)) { 
...
for (s in seq_len(nSpecies)) {
  ...
  if (inclSsbR[s]>0) {
    ...
    predObs<-SSB_R(s,y=(1:nYears.model)[recruitYears[s,]])
